...
public class allea {
  ...
  public static int getAlléa(int x){
    //if (x <= 0) { erreur.erreur("On ne peu tirer un nombre alléatoire qu'entre 2 borne positive, "+x+" ne convient pas","allea.getAlléa","On revoie 0"); return 0;}
    int xr = rand.nextInt(x);
    //debug.débogage(x + " a été tiré alléatoirement");
    return xr;
  }public static int getAllea(int x){return getAllea(x);}
 ...
 // plein de vieille fonctions qui viènent de la version 0.2.0
  ...
  // fluctuer
  // en %age
  public static int fluctuer(int x, byte a){ // a varit de 1 a 10;
