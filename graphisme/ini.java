...
public class ini {
  ...
  public static void initialiserFourmiTournées(){
    ...
    for (int i=1;i<len ;i++ ) {
      créerDBG("temporaire/F",i);
    }
  }
  public static void créerDBG(String nom, int i){
    String is = ""; if(i!=-1){is=i+"";}
    Img imgTemp = new Img(nom+is+".png");
    String rep = ""; if(str.nbrDeX(nom,'/')==0){rep="temporaire/";}
    String s = rep+nom+is;
    imgTemp.sauvegarde(s+"h"+".png");
    if (Main.getElementSurCarteOrientéAprèsDéplacement()){
      imgTemp = imgTemp.tourner();
      imgTemp.sauvegarde(s+"d"+".png");
      imgTemp = imgTemp.tourner();
      imgTemp.sauvegarde(s+"b"+".png");
      imgTemp = imgTemp.tourner();
      imgTemp.sauvegarde(s+"g"+".png");
    }
  }
