...
*@author Hydrolien
*/
public class FImageTree extends ImageTree {
  /**
  ...
  */
  ...
  public FImageTree copyStructure() {
    return (FImageTree)super.copyStructure(new FImageTree());
  }
  /**
  ...
  */
  public static FImageTree getScaledInstanceFromTree(FImageTree treeIn, int dim){
    ...
    FImageTree treeOut = treeIn.copyStructure();
    //insect
      ...
      for (int i=0;i<2 ;i++ ) { //imago ♂ & ♀
        if(nodeIn.getChildren(0)!=null && nodeIn.getChildren(0).getChildren(i)!=null){
          if (nodeIn.getChildren(0).getChildren(i).getContent()!=null) {
            ...
            biOut = Images.resize(biIn,FImages.taille(idSpecies+100,0,dim));
      ...
      for (int i=1;i<4 ;i++ ) { // other stade
        if(nodeIn.getChildren(i)!=null){
          if (nodeIn.getChildren(i).getContent()!=null) {
            ...
            biOut = Images.resize(biIn,FImages.taille(idSpecies+100,-i,dim));
    ...
    for (TreeNode<BufferedImage> nodeIn : antListIn) {
      ...
      for (int i=0; i<len; i++ ) { //imago ♂, ♀, minor, media, major, soldier etc
        ...
        if(nodeIn.getChildren(0)!=null && nodeIn.getChildren(0).getChildren(i)!=null){
          ...
          if(biIn!=null){ //if there is an ant
            int size = FImages.tailleFourmi(idSpecies,i,dim);
      ...
      for (int i=1;i<4 ;i++ ) { // other stade
        if(nodeIn.getChildren(i)!=null && nodeIn.getChildren(i).getContent()!=null){
          biIn = nodeIn.getChildren(i).getContent();
          biOut = Images.resize(biIn,FImages.taille(idSpecies,-i,dim));
