...
import fr.formiko.usual.modificationDeFichier;
import fr.formiko.usual.images.Images;
import fr.formiko.usual.images.Img;
import fr.formiko.usual.media.audio.AudioPlayer;
 ...
 * For exemple ./run.sh -cli will launch the game in cli mode when ./run.sh stats will never launch game but update statistique value on a
 * file.<br>
 * 
 * @author Hydrolien
...
public class launchOptions {
  ...
  public static void launchOptionsMajor(String args[]) {
    ...
    case "tradChar": {
      ...
      if (args[1].equals("all")) {
        ...
        if (id == -1) {
          id = chargerLesTraductions.getLanguage(args[1]);
        }
    ...
    case "testFont": {
      ...
      if (args[1].equals("all")) {
        ...
        try {
          File f = new File(Main.getFolder().getFolderStable() + Main.getFolder().getFolderBin() + "font/Insektofobiya.ttf");
          System.out.println(f.getAbsolutePath());
          Font fontLocal = Font.createFont(Font.TRUETYPE_FONT, f);
          canDisplayLanguages(fontLocal.getName());
          System.out.println();
        } catch (Exception e) {
          erreur.alerte("Local font not found");
        }
        for (String defaultFontName : List.of("Dialog", "DialogInput", "Monospaced", "Serif", "SansSerif")) {
          canDisplayLanguages(new Font(defaultFontName, Font.PLAIN, 1).getName());
        }
        System.out.println();
        Font fonts[] = GraphicsEnvironment.getLocalGraphicsEnvironment().getAllFonts();
        for (Font font : fonts) {
          canDisplayLanguages(font.getName());
        }
      } else {
  ...
  public static void canDisplayLanguages(String fontName) {
    ...
    int len = chargerLesTraductions.getTLangue().length;
    for (int i = 0; i < len; i++) {
      if (Translation.canDisplayLanguage(i, fontName)) {
        cpt++;
      }
      // else {
      // erreur.println("Can't display : " + chargerLesTraductions.getTLangue()[i]+ " "+);
      // }
    }
    String col = null;
    if (cpt == 0) {
  ...
  public static void printVersion() {
    ...
    erreur.info("Formiko version: " + folder.getVersion() + "   data version: " + folder.getCurentDataVersion() + "   music version: "
        + folder.getCurentMusicVersion());
  }
  /**
